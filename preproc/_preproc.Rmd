...
list(
  ...
  tar_target(
    ...
    data_valid |>
      ...
      ungroup() |>
      group_by(game_name, game_name_abbr, block) |>
      group_modify(
        possibly(
         ~ . |>
          pivot_wider(
        ...
        ~ if (nrow(.) < 100) {
          ...
          tibble(
            ...
            retest_change = .x |>
              filter(!Outlier) |>
              summarise(
                t.test(retest, test, paired = TRUE) |>
                  broom::tidy()
              ) |>
              mutate(
                p.format = format.pval(p.value, 2, 1e-4, scientific = FALSE),
                p.sig = cut(
                  p.value,
                  c(0, 1e-4, 1e-3, 1e-2, 0.1, 1),
                  c("****", "***", "**", "*", "")
                )
              ) |>
              mutate(
                summary_msg = str_glue(
                  "{round(estimate, 3)}({p.format}{p.sig})"
                )
              ) |>
              pull(summary_msg)
          )
