...
dend <- clust |> 
  ...
  set("labels_cex", 0.4)
plot(dend)
rect.dendrogram(dend, k = 3, border = 8)
```
...
在第一步中，我们对认知测试包含的所有任务具有代表性的指标进行探索性因素分析，并将factor数设置为1，意味着我们筛选出具有至少1个共同成分的任务作为训练组和迁移任务的备选。无法找出共同成分的任务，我们设置为控制组训练任务（factor的载荷低于0.3）。
