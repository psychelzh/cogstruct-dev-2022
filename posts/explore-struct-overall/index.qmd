...
test_retest_raw <- indices_trans |> 
  group_by(game_name, index_name, dimension) |> 
  group_modify(~ calc_test_retest(.)) |> 
  ungroup()
test_retest_trans <- indices_trans |>
  semi_join(game_needs_trans, by = c("game_name", "index_name")) |> 
  group_by(game_name, index_name, dimension) |> 
  group_modify(
    ~ calc_test_retest(
      .,
      name_test = "test_trans",
      name_retest = "retest_trans"
    ) |> 
      rename_with(~str_c(., "_trans"))
  ) |> 
  ungroup()
test_retest <- test_retest_raw |> 
  left_join(
    test_retest_trans, 
    by = c("game_name", "index_name", "dimension")
  )
...
test_retest |> 
  ...
  formattable(
    list(
      formattable::area(col = c(icc, icc_trans)) ~ 
        color_tile("white", "pink", na.rm = TRUE),
      game_name = formatter(
        "span",
        style = ~ if_else(
          coalesce(icc_trans, icc) < 0.5,
          "background:grey",
          NA_character_
        )
      )
...
indicators_remove <- test_retest |> 
  mutate(icc = coalesce(icc_trans, icc)) |> 
  filter(icc < 0.5) |> 
